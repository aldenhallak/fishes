...
class QiniuUploader {
  constructor(config = qiniuConfig) {
    ...
    // 验证配置
    if (!config.accessKey) {
      throw new Error('七牛云配置缺失: QINIU_ACCESS_KEY 未设置。请在 Vercel 后台设置环境变量。');
    }
    if (!config.secretKey) {
      throw new Error('七牛云配置缺失: QINIU_SECRET_KEY 未设置。请在 Vercel 后台设置环境变量。');
    }
    if (!config.bucket) {
      throw new Error('七牛云配置缺失: QINIU_BUCKET 未设置。请在 Vercel 后台设置环境变量。');
    }
