...
class QiniuUploader {
  constructor(config = qiniuConfig) {
    ...
    if (config.zone) {
      qnConfig.zone = qiniu.zone[config.zone] || qiniu.zone.Zone_z2;
    }
   ...
   * @param {string} key - 可选的自定义key
   * @returns {Promise<{path: string, url: string, key: string}>}
   */
  async uploadFile(fileData, filename = 'fish.png', key = null) {
