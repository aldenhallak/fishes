...
// 记录配置状态
console.log('[Hasura] Configuration status:');
console.log(`  HASURA_GRAPHQL_ENDPOINT: ${HASURA_GRAPHQL_ENDPOINT ? 'SET' : 'NOT SET'}`);
console.log(`  HASURA_ADMIN_SECRET: ${HASURA_ADMIN_SECRET ? 'SET' : 'NOT SET'}`);

if (!HASURA_GRAPHQL_ENDPOINT) {
  console.error('❌ HASURA_GRAPHQL_ENDPOINT not set in environment variables');
}

if (!HASURA_ADMIN_SECRET) {
  console.error('❌ HASURA_ADMIN_SECRET not set in environment variables');
}
 ...
 * @returns {Promise<object>} 完整的GraphQL响应 {data, errors}
 */
async function executeGraphQL(query, variables = {}) {
  // 检查环境变量
  if (!HASURA_GRAPHQL_ENDPOINT) {
    throw new Error('HASURA_GRAPHQL_ENDPOINT is not configured. Please set this environment variable in Vercel dashboard.');
  }
  
  if (!HASURA_ADMIN_SECRET) {
    throw new Error('HASURA_ADMIN_SECRET is not configured. Please set this environment variable in Vercel dashboard.');
  }
  ...
  try {
    ...
    return result; // Return full result with data and errors
  } catch (error) {
...
/**
 * 执行GraphQL查询 - 返回data部分（向后兼容）
 * @param {string} query - GraphQL查询语句
 * @param {object} variables - 查询变量
 * @returns {Promise<object>} 查询结果的data部分
 */
...
// 导出
module.exports = {
  executeGraphQL, // Returns full result {data, errors}
  query, // Returns result.data only
  mutation,
  queryHasura: query, // Alias for compatibility
  getFish,
  getFishById,
  updateFish,
  batchUpdateFish,
