...
async function getUserDailyGroupChatUsage(userId) {
    ...
    try {
        ...
        if (debugResult.data && debugResult.data.group_chat) {
            console.log(`[AI Fish Group Chat Usage] User ${userId} has ${count} group chats today (found ${debugResult.data.group_chat.length} records)`);
            // 检查是否有 conversation_id 为 NULL 的记录
            const nullConversationCount = debugResult.data.group_chat.filter(gc => !gc.conversation_id).length;
            if (nullConversationCount > 0) {
                console.warn(`[AI Fish Group Chat Usage] ⚠️ Found ${nullConversationCount} records with NULL conversation_id`);
            }
        }
    } catch (debugError) {
        console.warn('[AI Fish Group Chat Usage] Debug query failed:', debugError);
    }
