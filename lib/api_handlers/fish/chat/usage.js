...
async function getUserGroupChatLimit(userId) {
    ...
    if (result.errors) {
        ...
        const fallbackLimit = await getGlobalParamInt('free_daily_group_chat_limit', 5);
        return { limit: fallbackLimit, unlimited: false };
    }
    ...
    // Fallback to global param if not set or invalid
    const fallbackLimit = await getGlobalParamInt('free_daily_group_chat_limit', 5);
    console.log(`[AI Fish Group Chat] Using fallback limit from global param: ${fallbackLimit} (group_chat_daily_limit was: ${groupChatDailyLimit})`);
    return { limit: fallbackLimit, unlimited: false };
}
