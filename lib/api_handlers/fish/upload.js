...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const form = formidable({
      ...
      maxFields: 1000,
      hashAlgorithm: false // 禁用hash计算以提高速度
      // 移除 enabledPlugins - 这会导致错误
    });

    console.log('[上传API] 开始formidable解析...');
    console.log('[上传API] 请求方法:', req.method);
    console.log('[上传API] 请求URL:', req.url);
    ...
    const [fields, files] = await new Promise((resolve, reject) => {
      ...
      form.parse(req, (err, fields, files) => {
        ...
        if (err) {
          ...
          console.error('[上传API] 错误代码:', err.code);
          console.error('[上传API] 错误消息:', err.message);
          reject(err);
        } else {
          console.log('[上传API] 解析成功，耗时:', parseTime, 'ms');
          console.log('[上传API] files:', Object.keys(files));
          console.log('[上传API] fields:', Object.keys(fields));
