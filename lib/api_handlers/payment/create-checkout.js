...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const priceId = billingPeriod === 'yearly' 
      ? memberType.stripe_price_id_yearly 
      : memberType.stripe_price_id_monthly;
    ...
    if (!priceId) {
      ...
      const price = await stripe.prices.create({
        unit_amount: Math.round((billingPeriod === 'yearly' ? memberType.yearly_price : memberType.monthly_price) * 100),
        currency: 'usd',
        recurring: {
        ...
        product_data: {
          name: productName,
          description: `${memberType.name} membership plan`
        }
      });
