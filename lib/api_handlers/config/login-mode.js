...
module.exports = async function handler(req, res) {
  ...
  // 仅在开发环境提供（允许未设�?NODE_ENV 的情况，视为开发环境）
  ...
  try {
    // 从环境变量读取登录模�?    const loginMode = process.env.LOGIN_MODE || 'MANUAL';
    
    // 读取用户凭据（兼�?DEF_USER �?DEF_USE�?    const email = process.env.DEF_USER || process.env.DEF_USE || null;
