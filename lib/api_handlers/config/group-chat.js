 ...
 * 
 * 返回群聊功能配置（仅限开发环境）
 * �?GROUP_CHAT = ON 时，默认开启群聊；= OFF 时默认关�? */
...
module.exports = async function handler(req, res) {
  ...
  try {
    // 从环境变量读取群聊模式（默认 OFF�?    const groupChatMode = process.env.GROUP_CHAT || 'OFF';
    const isEnabled = groupChatMode.toUpperCase() === 'ON';

    // 从全局参数表读取群聊时间间隔（单位：秒，转换为分钟�?    // group_chat_interval_s: 群聊间隔（秒），默认 30 �?= 0.5 分钟，但通常设置�?300 �?= 5 分钟
